...
import { Product, ProductCreate } from "../data/data";

type ProductRecord = Record<keyof ProductCreate, Yup.AnySchema>;

const ProductSchema = Yup.object().shape<ProductRecord>({
    title: Yup.string().min(2, "Title must be longer than 2 letters").max(25, "max 25 letters").required("First name field cannot be empty"),
    price: Yup.number().min(1, "Cost must be more than 0").strict().required("Price field cannot be empty"),
    imageUrl: Yup.string().url().min(5).max(100).notRequired()
});
